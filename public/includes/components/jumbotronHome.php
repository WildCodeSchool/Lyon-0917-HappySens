<div id="index-banner" class="parallax-container-home">
    <div class="section no-pad-bot">
        <div class="col s12">
            <div class="headerTitleHome center" id="headerTitle">
                <h1 class="header center amber-text">Bienvenue sur Happy Sens</h1>
                <div class="row">
                    <h5 class="header center white-text">Mettez du happy dans votre entreprise</h5>
                </div>
                <div id="goDown" class="goDown">
                    <a class="btn btn-floating btn-large pulse pink darken-2" href="#cardCategories"><i class="material-icons">expand_more</i></a>
                </div>
            </div>
        </div>
    </div>
</div>

